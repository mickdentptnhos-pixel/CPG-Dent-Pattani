...
/* =====================================================================
   เบาหวาน (DM): ตัวช่วยแปลงค่าน้ำตาลเป็นระดับความเสี่ยง
   — รองรับทั้งกรณีมีผล HbA1c และกรณีไม่มี (ใช้ FBS / น้ำตาลปลายนิ้ว POC)
   ===================================================================== */
const DM_FBS_TO_LEVEL = { fhypo: "hypo", f1: "l1", f2: "l2", f3: "l3", f4: "l4" };
const DM_FBS_BANDS    = { f1: "70–199", f2: "200–249", f3: "250–299", f4: "≥ 300" };
const DM_HBA1C_DESC   = {
  l1: "HbA1c ≤ 7.0% (eAG ≤ 154 mg/dL)",
  l2: "HbA1c 7.1–8.9% (eAG 155–211 mg/dL)",
  l3: "HbA1c 9.0–9.9% (eAG 212–239 mg/dL)",
  l4: "HbA1c ≥ 10.0% (eAG ≥ 240 mg/dL)"
};
/* ระดับความเสี่ยงที่ resolve แล้ว: l1–l4 หรือ 'hypo' (น้ำตาลต่ำ ต้องแก้ก่อน) */
const dmLevel = (a) => (a.glucoBasis === "fbs" ? DM_FBS_TO_LEVEL[a.fbs] : a.hba1c);
/* คำบรรยายค่าน้ำตาลสำหรับใส่ในผลลัพธ์/ใบ consult */
const dmGlyDesc = (a) => {
  if (a.glucoBasis === "fbs") { const b = DM_FBS_BANDS[a.fbs]; return b ? "น้ำตาลปลายนิ้ว/FBS " + b + " mg/dL" : ""; }
  return DM_HBA1C_DESC[a.hba1c] || "";
};
...
const PROBLEMS = {
  ...
  dm: {
    ...
    questions: [
      ...
      {
        id: "glucoBasis",
        label: "มีผลตรวจระดับน้ำตาลแบบใด?",
        type: "chipChoice",
        chipStack: true,
        clears: ["hba1c", "fbs", "procedure"],
        options: [
          { value: "hba1c", dot: "🧪", text: "มีผล HbA1c", sub: "ค่าที่เหมาะที่สุดสำหรับวางแผน — สะท้อนการควบคุมเฉลี่ย 2–3 เดือน" },
          { value: "fbs",   dot: "🩸", text: "ไม่มี HbA1c — ใช้ FBS / น้ำตาลปลายนิ้ว (POC)", sub: "เจาะน้ำตาลปลายนิ้วด้วย glucometer ก่อนทำหัตถการ (ตรวจง่าย ทำได้ทันทีในคลินิก)" }
        ],
        showIf: (a) => a.proc && a.proc !== "emergency"
      },
      {
        ...
        showIf: (a) => a.proc && a.proc !== "emergency" && a.glucoBasis === "hba1c"
      },
      {
        id: "fbs",
        label: "ระดับน้ำตาลปลายนิ้ว / FBS ก่อนทำหัตถการ (mg/dL)",
        type: "chipChoice",
        chipStack: true,
        clears: ["procedure"],
        tooltips: {
          fhypo: { head: "ภาวะน้ำตาลต่ำ (Hypoglycemia)", body: "BG < 70 mg/dL — ต้องแก้ไขก่อนทำหัตถการเสมอ<br><b>กฎ 15 กรัม:</b> คาร์โบไฮเดรตดูดซึมเร็ว 15–20 กรัม แล้วเจาะซ้ำใน 15 นาที" },
          f4:    { head: "สัญญาณ Diabetic Ketoacidosis (DKA)", body: "กระหายน้ำรุนแรง · ปัสสาวะบ่อย · <b>ลมหายใจกลิ่นผลไม้</b> · สับสน<br>หากพบ → งดทันตกรรม ส่งระบบการแพทย์ฉุกเฉินทันที" }
        },
        options: [
          { value: "fhypo", dot: "🔻", text: "< 70",      sub: "ภาวะน้ำตาลต่ำ — แก้ไขก่อน (กฎ 15 กรัม) แล้วเจาะซ้ำ" },
          { value: "f1",    dot: "✅", text: "70–199",    sub: "อยู่ในเกณฑ์ปลอดภัยสำหรับหัตถการ — เทียบเท่าคุมได้ดี" },
          { value: "f2",    dot: "🟢", text: "200–249",   sub: "สูงปานกลาง — ทำได้ เฝ้าระวังน้ำตาล/การหายของแผล" },
          { value: "f3",    dot: "🟠", text: "250–299",   sub: "สูง (คุมไม่ดี) — ระวังสูง ชะลองาน elective" },
          { value: "f4",    dot: "⛔", text: "≥ 300",     sub: "สูงวิกฤต — งดหัตถการ (เว้นช่วยชีวิต) เฝ้าระวัง DKA" }
        ],
        showIf: (a) => a.proc && a.proc !== "emergency" && a.glucoBasis === "fbs"
      },
      {
        id: "procedure",
        label: "เลือกประเภทหัตถการ",
        ...
        showIf: (a) => { const lv = dmLevel(a); return a.proc && a.proc !== "emergency" && lv && lv !== "l4" && lv !== "hypo"; }
      }
    ],
    evaluate(a, ctx) {
      const proc = ctx.proc;
      ...
      const lv = dmLevel(a);
      const glyDesc = dmGlyDesc(a);
      const isFbs = a.glucoBasis === "fbs";
      const fbsCaveat = isFbs
        ? "\n\nℹ️ ค่านี้เป็นภาพรวมชั่วขณะ — สำหรับงานผ่าตัด/ฝังรากเทียมตามนัด แนะนำตรวจ HbA1c ยืนยันการควบคุมระยะยาวก่อน (หากทำได้)"
        : "";
      if (!lv) return { status: null };

      /* ---------- BG < 70 : แก้ไขน้ำตาลต่ำก่อน ---------- */
      if (lv === "hypo") {
        return {
          status: "amber",
          title: "แก้ไขภาวะน้ำตาลต่ำก่อน แล้วเจาะประเมินซ้ำ",
          message:
            "น้ำตาลปลายนิ้ว < 70 mg/dL — ภาวะน้ำตาลต่ำ ห้ามเริ่มหัตถการจนกว่าจะแก้ไข\n" +
            "• กฎ 15 กรัม: คาร์โบไฮเดรตดูดซึมเร็ว 15–20 กรัม (น้ำผลไม้ 120–180 mL หรือกลูโคสอัดเม็ด 3–4 เม็ด) แล้วเจาะซ้ำใน 15 นาที ทำซ้ำจนกว่า BG ≥ 70 mg/dL\n" +
            "• เมื่อระดับน้ำตาลกลับสู่เกณฑ์ปลอดภัยแล้ว จึงเจาะประเมินซ้ำและวางแผนหัตถการตามค่าที่ได้\n" +
            "• หากหมดสติ → ห้ามป้อนทางปาก · เรียก 1669 · 50% Dextrose 20–50 mL IV หรือ Glucagon 1 mg IM"
        };
      }

      /* ---------- ระดับสูงสุด : งดทุกหัตถการ ---------- */
      if (lv === "l4") {
        return {
          status: "red",
          title: "งดหัตถการ — ส่งควบคุมเบาหวานก่อน (Severe uncontrolled)",
          message:
            glyDesc + " — คุมไม่ได้/วิกฤต เสี่ยงสูงต่อ DKA แผลไม่หาย และการติดเชื้อลุกลาม\n" +
      ...
      if (lv === "l1") {
        if (pr === "nonSurg" || pr === "surgery") {
          return {
            ...
            title: "ให้การรักษาได้ตามปกติ",
            message:
              glyDesc + " — คุมได้ดี ไม่มีข้อห้ามในการรักษา ทำหัตถการได้ทุกประเภทเทียบเท่าคนปกติ\n" +
              (pr === "surgery" ? "คำแนะนำสำหรับงานศัลยกรรม:\n" + surgicalCare + "\n(ไม่จำเป็นต้องให้ยาปฏิชีวนะป้องกันในผู้ป่วยที่คุมน้ำตาลได้ดี)" + fbsCaveat : "เน้นดูแลสุขอนามัยช่องปากและควบคุมโรคปริทันต์") +
              generalCare
          };
        }
        /* implant */
        return {
          ...
          message:
            glyDesc + " (Good control ตาม DGI/DGZMK) — ความเสี่ยง osseointegration ล้มเหลวใกล้เคียงประชากรทั่วไป\n" +
            "• ผ่าตัดฝังรากเทียม/ปลูกกระดูกได้\n" +
            surgicalCare +
            "\n• ตรวจ POC glucose ซ้ำในวันผ่าตัด · เฝ้าระวังการหายของแผลรอบรากเทียม" +
            fbsCaveat + generalCare
        };
      }
      ...
      if (lv === "l2") {
        if (pr === "nonSurg") {
          return {
            ...
            message:
              glyDesc + " — คุมได้ปานกลาง ทำงานทันตกรรมที่ไม่ใช่การผ่าตัดได้ แต่ต้องเฝ้าระวังระดับน้ำตาลใกล้ชิด หากมีอาการผิดปกติให้หยุดทันที" +
        ...
        if (pr === "surgery") {
          return {
            ...
            message:
              glyDesc + " — ทำงานศัลยกรรมได้ โดยไม่ต้องหยุด/รอ แต่ระบบภูมิคุ้มกันและการหายของแผลด้อยลง\n" +
              surgicalCare + apNote + fbsCaveat + generalCare + hypoNote
          };
        }
        ...
        if (proc === "elective") {
          return {
            ...
            message:
              glyDesc + " (Moderate ตาม DGI/DGZMK) — เสี่ยงระดับกลางถึงสูงต่อ crestal bone loss และ peri-implantitis\n" +
              "• เป็นงาน elective — แนะนำประสานแพทย์ควบคุมน้ำตาลให้ HbA1c < 8% (ยิ่งใกล้ ≤ 7% ยิ่งดี) ก่อนผ่าตัดฝังรากเทียม\n" +
              "• หากผู้ป่วยยืนยันทำ: แจ้งความเสี่ยง ขอ informed consent · ตรวจ POC glucose วันผ่าตัด · ให้ยาปฏิชีวนะป้องกัน · นัดติดตามใกล้ชิด\n" +
              surgicalCare + fbsCaveat + generalCare,
            consultBody: true
          };
        }
        return {
          ...
          message:
            glyDesc + " — เสี่ยงระดับกลางต่อการหายของกระดูกรอบรากเทียม\n" +
      ...
      if (lv === "l3") {
        if (pr === "nonSurg") {
          return {
            ...
            message:
              glyDesc + " — เข้าสู่โซนคุมไม่ดี ทำงานทันตกรรมพื้นฐาน (ไม่ผ่าตัด) ได้ แต่ต้องระวังสูงและเฝ้าระวังน้ำตาลใกล้ชิด\n" +
        ...
        if (pr === "surgery") {
          if (proc === "urgency") {
            return {
              ...
              message:
                glyDesc + " + เร่งด่วน (เช่น ติดเชื้อ/ปวดรุนแรง) — ให้การรักษาแบบประคับประคอง/ถอนฟัน/ระบายหนองได้\n" +
                surgicalCare +
          ...
          return {
            ...
            message:
              glyDesc + " — งานศัลยกรรมแบบนัดหมาย (โดยเฉพาะการเปิดแผ่นเหงือก/กระดูกปริทันต์) ควรชะลอไว้ก่อน\n" +
              "• ส่งปรึกษาแพทย์เพื่อควบคุมน้ำตาล แล้วนัดทำหัตถการใหม่เมื่อควบคุมน้ำตาลดีขึ้น",
            consultBody: true
          };
        }
        ...
        return {
          ...
          message:
            glyDesc + " — ไม่แนะนำผ่าตัดฝังรากเทียม/ปลูกกระดูกในภาวะนี้ (osseointegration ล้มเหลวสูง)\n" +
            "• เลื่อนหัตถการออกไป · ส่งปรึกษาแพทย์เพื่อควบคุมน้ำตาลให้ HbA1c < 8% (ควรใกล้ ≤ 7%) ก่อนวางแผนฝังรากเทียม",
          consultBody: true
        };
      }
